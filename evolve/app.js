...
(function () {
  ...
  IC.LAYOUTS.forEach((L, i) => {
    ...
    card.innerHTML = `${wireHTML(L.wire)}${L.rec ? '<div class="ov-rec">★ Recommended</div>' : ''}${L.easy ? '<div class="ov-rec easy">✓ passes the five tests</div>' : ''}<div class="ov-n">${i + 1}. ${L.name}<small>· ${L.app}</small></div><div class="ov-d">${L.note}</div><div class="ov-k">${L.wire} structure</div>`;
